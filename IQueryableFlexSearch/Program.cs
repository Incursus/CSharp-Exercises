...
namespace IQueryableFlexSearch
{
    class Program
    {
        static void Main()
        {
            ...
            string gradeAverage = Console.ReadLine().Trim();
            ...
            if (firstName != "")
            {
                studentQuery = students.AsQueryable()
                        .Where(student => student.FirstName.StartsWith(firstName, StringComparison.CurrentCultureIgnoreCase));
            }

            if (lastName != "")
            {
                studentQuery = students.AsQueryable().Where(student => student.LastName.StartsWith(lastName, StringComparison.CurrentCultureIgnoreCase));
            }
